...
export default function AnalysisResult({
  ...
  const SectionHeader = ({ icon: Icon, title, badge }: any) => (
    <div className="flex items-center justify-between border-b border-slate-200 pb-4 mb-6">
      ...
      {badge && (
        <Badge className="bg-slate-100 text-slate-600 rounded-none text-[8px] font-black uppercase border-slate-200">
          {badge}
        </Badge>
      )}
    </div>
  ...
  return (
    <div
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b-4 border-slate-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="rounded-none border-2 border-slate-900 bg-white p-8 space-y-6 relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div className="flex items-end justify-between gap-4">
              <div className="text-center flex-1">
                ...
                <h4 className="text-6xl font-black text-slate-900">
                  {analysis.win_probability?.your_team || 0}%
                </h4>
              </div>
              <div className="h-16 w-px bg-slate-200 mb-2" />
              <div className="text-start flex-1">
                <p className="text-[10px] font-black text-slate-400 uppercase ">
                  {matchInfo.opponent_team || teams.opponent_team?.team_name}
                </p>
                <h4 className="text-6xl font-black text-blue-600">
            ...
            <div className="space-y-3 pt-6 border-t border-slate-100">
              {analysis.win_probability?.key_factors?.map(
                (f: string, i: number) => (
                  <div
                    key={i}
                    className="flex gap-2 text-sm font-medium text-slate-500  tracking-tight">
        ...
        <Card className="rounded-none border-2 border-slate-900 bg-slate-900 text-white p-8 group overflow-hidden relative">
          <div className="absolute bottom-0 right-0 p-4 opacity-10 translate-y-4 group-hover:translate-y-0 transition-transform">
          ...
          <div className="relative z-10 space-y-4 h-full flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <h3 className="text-4xl font-black leading-none uppercase  ">
        ...
        <Card className="rounded-none border-2 border-slate-900 bg-white p-8 space-y-6">
          <p className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
          ...
          {!weather || Object.keys(weather).length === 0 ? (
            ...
            <>
              ...
              <div className="p-2 bg-slate-50 border border-slate-100 flex gap-3">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {analysis.key_matchups?.map((m: any, i: number) => (
            <Card
              key={i}
              className="rounded-none border-2 border-slate-200 bg-white p-6 relative group overflow-hidden hover:border-blue-600 transition-colors">
              <div className="absolute top-0 left-0 w-1 h-full bg-slate-200 group-hover:bg-blue-600 transition-all" />
              <div className="space-y-4">
                <h5 className="font-black text-sm uppercase tracking-tight text-slate-900">
                  {m.battle}
                </h5>
                <div className="flex flex-col gap-2">
                  <Badge className="bg-blue-50 text-blue-700 border-none rounded-none text-[12px] font-medium  px-2 w-fit">
                    Advantage: {m.advantage}
                  </Badge>
                  <p className="text-xs text-slate-500 font-medium   border-t border-slate-50 pt-2">
                    {m.reasoning}
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card className="rounded-none border-2 border-slate-900 bg-amber-50 p-8 space-y-4">
          <SectionHeader
            icon={CloudRain}
          ...
          <p className="text-xs font-semibold text-slate-600 leading-relaxed border-t border-amber-200 pt-4">
            {analysis.weather_analysis?.dls_strategy}
          </p>
        </Card>

        <Card className="rounded-none border-2 border-slate-900 bg-white p-8 space-y-4">
          <SectionHeader
            icon={TrendingUp}
          ...
          <p className="text-xs font-medium text-slate-600   bg-slate-50 p-3 ">
      ...
      <div className="space-y-12">
        {[
          ...
          <div
            key={idx}
            className="space-y-8 p-1 bg-slate-50 border border-slate-200">
            <div className="bg-white p-8 space-y-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-2 border-slate-100 pb-6">
                <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter flex items-center gap-3">
                  <span
                    className={cn(
                      "w-3 h-8",
                      idx === 0 ? "bg-blue-600" : "bg-slate-900",
                    )}
                  />
                ...
                <Badge className="bg-slate-100 text-slate-400 border-none rounded-none text-[9px] font-bold">
                  REGISTRY_PATH: TEAMS.
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  ...
                  <div key={pi} className="space-y-2">
                    ...
                    <p className="text-sm font-medium text-slate-600  min-h-[60px]">
              ...
              <div className="space-y-6 pt-10 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {block.lineup?.map((p: any, li: number) => (
                    <div
                      key={li}
                      className="p-4 bg-slate-50 border border-slate-100 hover:border-blue-400 transition-all flex flex-col justify-between group">
                      <div className="space-y-2">
                        <div className="flex justify-between items-start">
                          <span className="text-xs font-black text-slate-900 group-hover:text-blue-600 transition-colors uppercase leading-none">
                            {p.name}
                          </span>
                          <span className=" font-black bg-white px-1.5 py-0.5 border border-slate-200 uppercase">
                            {li + 1}
                          </span>
                        </div>
                        <p className="t font-bold text-slate-400 uppercase tracking-tight">
                          {p.role}
                        </p>
                        <p className=" text-slate-500  font-medium text-sm md:pt-2 tracking-tight">
                          "{p.reasoning}"
                        </p>
                      </div>
                    </div>
              ...
              <div className="pt-10">
                <button
                  ...
                  className="w-full flex items-center justify-between p-4 bg-slate-900 text-white rounded-none hover:bg-slate-800 transition-colors">
                    ...
                    <table className="w-full text-[10px] text-left">
                      <thead>
                        <tr className="bg-slate-100 border-b border-slate-200">
                          <th className="p-3 text-black text-[11px] font-black uppercase tracking-wider">
                            Player No
                          </th>
                          <th className="p-3 text-black text-[11px] font-black uppercase tracking-wider">
                            Name
                      ...
                      <tbody>
                        {block.allPlayers?.map((player: any, pi: number) => (
                          <tr
                            ...
                            <td className="p-3  text-slate-400 group-hover:text-blue-500">
                            ...
                            <td className="p-3">
                              <Badge className="bg-emerald-100 text-emerald-800 rounded-none border-none text-[7px] font-black uppercase tracking-widest px-1">
                                STABLE
                              </Badge>
                            </td>
                          </tr>
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card className="rounded-none border-2 border-slate-900 bg-white p-10">
          <SectionHeader
            icon={Activity}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-6">
              ...
              {analysis.critical_factors?.map((f: string, i: number) => (
                <div
                  key={i}
                  className="flex gap-4 items-start p-3 border border-slate-50 hover:border-blue-100 transition-all">
                  <CheckCircle2 className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
                  <p className="text-sm font-bold text-slate-600 ">{f}</p>
                </div>
              ))}
            </div>
